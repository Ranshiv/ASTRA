"""Hubble Space Telescope observation-metadata connector via MAST CAOM.

HST observations are pointed image/spectral products, not a survey light-curve
stream, so the connector exposes observation metadata only.  Discovering that
an HST product covers a candidate is itself evidence; retrieving the product is
a separate bounded job.
"""
...
__all__ = ["HubbleConnector", "parse_rows"]


class HubbleConnector(SurveyConnector):
    name = "Hubble"
    ...
    def fetch_light_curves(self, source: SourceRef) -> list[LightCurve]:
        # HST products are images and spectra.  Any time series worth having is
        # built by a dedicated extraction job, not synthesised here.
        return []
