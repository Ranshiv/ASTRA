"""Hubble connector contract: cone search parsing, capabilities, no-op fetch."""
...
from astra.surveys.hubble import HubbleConnector, parse_rows

VALID_PAYLOAD = {
    "status": "COMPLETE",
    "fields": [{"name": "obsid"}, {"name": "s_ra"}, {"name": "s_dec"}],
    "data": [
        {"obsid": "2003123456", "obs_id": "hst_09490_01", "s_ra": 180.122,
         "s_dec": 22.411, "instrument_name": "ACS/WFC", "filters": "F606W",
         "t_min": 52251.5, "t_exptime": 1200.0, "dataproduct_type": "image"},
        {"obs_id": "hst_09490_02", "s_ra": 180.130, "s_dec": 22.420},
    ],
}
...
class TestHubbleConnector:
    ...
    def test_cone_search_parses_valid_rows(self, monkeypatch, cone: ConeQuery):
        ...
        sources = HubbleConnector().cone_search(cone, limit=10)
        assert len(sources) == 2
        assert sources[0].survey == "Hubble"
        assert sources[0].object_id == "2003123456"
        assert sources[0].ra_deg == pytest.approx(180.122)
        assert sources[0].extra["instrument"] == "ACS/WFC"
        # second row falls back to obs_id when obsid is absent.
        assert sources[1].object_id == "hst_09490_02"
    ...
    def test_cone_search_skips_rows_missing_position(self, monkeypatch, cone: ConeQuery):
        ...
        assert HubbleConnector().cone_search(cone) == []
    ...
    def test_cone_search_pages_until_a_short_page(self, monkeypatch, cone: ConeQuery):
        ...
        sources = HubbleConnector().cone_search(cone, limit=10_000)
    ...
    def test_cone_search_clamps_limit(self, monkeypatch, cone: ConeQuery):
        ...
        monkeypatch.setattr(netclient, "get", fake_get)
        HubbleConnector().cone_search(cone, limit=10_000)
        request = json.loads(captured["params"]["request"])
        assert request["pagesize"] == 200
        assert captured["provider"] == "mast"

    def test_cone_search_filters_to_hst_collection(self, monkeypatch, cone: ConeQuery):
        captured: dict = {}
        ...
        monkeypatch.setattr(netclient, "get", fake_get)
        HubbleConnector().cone_search(cone)
        request = json.loads(captured["params"]["request"])
    ...
    def test_fetch_light_curves_returns_empty(self):
        source = SourceRef(survey="Hubble", object_id="1", ra_deg=0.0, dec_deg=0.0)
        assert HubbleConnector().fetch_light_curves(source) == []
